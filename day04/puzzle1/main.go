...
func main() {
	start := time.Now()
	guardData := []string{"[1518-03-18 00:03] Guard #3529 begins shift",
	...
	form := "2006-01-02 04:05"

	for _, row := range guardData {
		row := strings.Trim(row, "[")
		splitData := strings.Split(row, "]")
		//fmt.Println("Timestamp:",splitData[0])
		timestamp, _ := time.Parse(form, strings.TrimSpace(splitData[0]))

		events[timestamp] = splitData[1]
	}

	// Can't sort a map. Need to store in a slice and sort the slice, then move that in to a map to have a
	// key value store of the data to work with
	fmt.Println(events)
	sort.Sort(events)
